class StageOfGrowthsController < ApplicationController
  ...
  def new
    @stage_of_growth = @task.stage_of_growths.new
    # @grow_media = @task.grow_medium.new(params.require(:grow_media).permit(:brand, :nutrients_added, :task_id))
  end

  def create
    @stage_of_growth = @task.stage_of_growths.new(stage_of_growth_params)
    if @stage_of_growth.save
      flash[:notice] = 'Stage of Growth successfully created'
      redirect_to task_stage_of_growths_path
    else
      render :new
    end
  end
  ...
  def edit
    @stage_of_growth = @task.stage_of_growths.find(params[:id])
  end
  ...
  def stage_of_growth_params
    params.require(:stage_of_growth).permit(:stage, :plant_date, :pot_size, :grow_medium, :task_id)
  end
